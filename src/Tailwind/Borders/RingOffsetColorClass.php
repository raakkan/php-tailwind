...
class RingOffsetColorClass extends AbstractTailwindClass
{
    private $isArbitrary;
    private $opacity;
    ...
    public function toCss(): string
    {
        ...
        $css = ".ring-offset-{$classValue}";
        if ($this->opacity !== null) {
            $css .= "\\/{$this->opacity}";
        }
        $css .= " {--tw-ring-offset-color: {$colorValue};}";
    ...
    private function getColorValue(): string
    {
        ...
        $colorName = str_replace(['ring-offset-'], '', $this->value);
    ...
    {
        if (preg_match('/^ring-offset-((?:\[.+\]|inherit|current|transparent|black|white|slate|gray|zinc|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)(?:-\d{1,3})?(?:\/[0-9]+)?)$/', $class, $matches)) {
            $value = $matches[1];
