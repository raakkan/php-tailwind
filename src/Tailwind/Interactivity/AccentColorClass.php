...
class AccentColorClass extends AbstractTailwindClass
{
    private $isArbitrary;
    ...
    public function toCss(): string
    {
        ...
        $css = ".accent-{$classValue}";
        if ($this->opacity !== null) {
            $css .= "\\/{$this->opacity}";
        }
        $css .= " {accent-color: {$colorValue};}";
    ...
    private function isValidValue(): bool
    {
        ...
        $validValues = array_merge(array_keys($this->getColors()), ['inherit', 'current', 'transparent', 'auto']);
    ...
    {
        if (preg_match('/^accent-((?:\[.+\]|inherit|current|transparent|auto|black|white|slate|gray|zinc|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)(?:-\d{1,3})?(?:\/[0-9]+)?)$/', $class, $matches)) {
            $value = $matches[1];
